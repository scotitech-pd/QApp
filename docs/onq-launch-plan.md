...
| S4.5 | Mobile | Release APK built locally (`~/Desktop/OnQ-release.apk`, 116 MB universal ABI — add `abiFilters arm64-v8a` or ship an AAB to Play for a ~35 MB download); iOS project generated for archive; TestFlight upload = founder's App Store Connect session | ✅ APK · 🔄 iOS upload founder |

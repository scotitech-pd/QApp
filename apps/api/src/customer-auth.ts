...
export async function verifyGoogleIdToken(idToken: string) {
  ...
  const info = (await response.json()) as { aud: string; sub: string; email?: string; email_verified?: string; given_name?: string; exp: string };
  ...
  return { providerId: info.sub, email: info.email_verified === "true" ? info.email ?? null : null, firstName: info.given_name ?? null };
}
...
export async function signInCustomer(
  ...
  identity: { providerId: string; email: string | null; firstName?: string | null }
) {
  const providerField = provider === "apple" ? "appleUserId" : "googleUserId";

  let customer = await prisma.customer.findFirst({ where: { [providerField]: identity.providerId } });

  if (!customer && identity.email) {
    customer = await prisma.customer.findFirst({ where: { email: identity.email } });
  }
  ...
  customer = customer
    ? await prisma.customer.update({
        ...
        data: { [providerField]: identity.providerId, email: customer.email ?? identity.email, firstName, lastSignInAt: new Date() }
      })
    : await prisma.customer.create({
        data: { firstName, email: identity.email, [providerField]: identity.providerId, lastSignInAt: new Date() }
      });

  return { token: createCustomerToken(customer.id), profile: serializeCustomer(customer) };
}

function serializeCustomer(customer: { id: string; firstName: string; email: string | null; phone: string | null; appleUserId: string | null; googleUserId: string | null }) {
  return {
    id: customer.id,
